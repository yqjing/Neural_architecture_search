...
def genetic_algorithm(train_data, validation_data, generations=200, population_size=20, to_keep=5, train_epoches = 20, gen_limit = 300, device = 'cpu', load=True):
    if load == False:
    ...
    while len(best_ed_20) <= to_keep and (outer, inner) != (1, 1):
        ...
        for generation in tqdm(range(generations)):
            ...
            if abs(generation - best_gen) >= 500 and len(best_ed_20) < to_keep:
                # best_ed_20.append(best_child.net_ed)
                # population = initialize_population(size=population_size, device = device)
                ...
                if patience > 2:
                    inner += 1
                    best_ed_20.append(best_child.net_ed)
                    best_ed_20_perf.append(best_child.performance_history)
                    population = initialize_population(size=population_size, device = device)
                    patience = 0

                print(f"# restart: progress {outer} {inner}  =====================================")
                for node in population:
                    node.assign_gen(0)
                if len(best_ed_20) == to_keep:
                    pass
                else:
                    break

            elif generation == gen_limit and len(best_ed_20) < to_keep:
                best_ed_20.append(best_child.net_ed)
                best_ed_20_perf.append(best_child.performance_history)
                population = initialize_population(size=population_size, device = device)
                inner += 1
            ...
            if len(best_ed_20) == to_keep:
                ...
                inner = 0
                outer += 1
                print(f"best_ed_20 is {best_ed_20}")
                torch.save(best_ed_20, "saved_encodings/best_ed_20.pkl")
                best_ed_20 = []

                best_id_ed_20 = torch.load("saved_encodings/best_ed_20.pkl")
                torch.manual_seed(2809)
                best_id = [Network(ed, device = device) for ed in best_id_ed_20]
                for idx, node in enumerate(best_id):
                    node.assign_gen(0)
                    node.performance_history = best_ed_20_perf[idx]
                population = crossover_and_mutate(best_id, population_size=population_size, to_keep=to_keep, gen = 0, device = device)
                break
